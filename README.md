...
`v0.3.5` — **Launch-path completion + bulk-ops infrastructure + TeamViewer client-version pill.** Three contained tasks ship together. (1) `MainViewModel.LaunchEntry` now plumbs through to the byte[] launcher overload from v0.3.4 — `EntryRepository.LoadEntryPasswordBytes` / `LoadEntryProxyPasswordBytes` feed `TeamViewerLauncher.Launch(entry, byte[]?, byte[]?, options)`, which zeros buffers via `try/finally + ZeroMemory` after argv compose. Clipboard-password mode and folder-default-inheritance flows fall back to the legacy string path automatically. (2) New `TeamViewerVersionDetector` reads the installed TeamViewer client version (registry then FileVersionInfo on the resolved `TeamViewer.exe`); status bar shows the version + a yellow "Update available" pill when the detected version is below 15.74.5 (CVE-2026-23572 baseline). Operator-side remediation guidance — TeamStation orchestrates the installed client and does not auto-update. (3) Bulk multi-select infrastructure on the connection tree: Ctrl-click toggles `IsMultiSelected` on individual nodes, plain click clears the selection (existing single-select semantics preserved). New `Bulk Pin (N)` / `Bulk Unpin (N)` context-menu items appear when 2+ entries are multi-selected. The infrastructure (TreeNode flag, `MainViewModel.SelectedNodes`, the click-handler) is the foundation for `BulkSetTag` / `BulkSetProxy` / `BulkSetMode` in v0.3.6. Test count 398 → 422, build clean at 0 warnings / 0 errors. See [CHANGELOG.md](CHANGELOG.md).
...
- Embedded 500-entry log panel with auto-scroll, severity-coloured rows, NDJSON export, and a launch-latency histogram
